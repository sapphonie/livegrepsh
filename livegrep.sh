...
LG_STORAGE_DIR=/home/sappho/livegrep_store/
...
iterateNonGithubDirectories()
{
    ...
    pushd src_stuff                 || exit 1

    tmpFile="${LG_STORAGE_DIR}/lg_local_.json"
    ...
    for thisdir in */ ; do
        ...
        {
            printf '\n        {'
            printf '\n          "path": "/data/src_stuff/%s"', "${realActualDir}"
            printf '\n          "name": "src_stuff/%s"', "${realActualDir}"
            printf '\n          "revisions": ['
            printf '\n            "HEAD"'
            printf '\n          ]'
            printf '\n        }%s' "${trailingComma}"
...
combineIndexes()
{
    ...
    spruce merge livegrep.json lg_src_stuff.json | spruce json | jq -S -M > lg_merged.json
